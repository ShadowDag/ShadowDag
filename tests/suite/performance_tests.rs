...
mod tests {
    ...
    fn genesis_block() -> Block {
        Block {
            header: BlockHeader::new_with_defaults(
                1,
                "perf_genesis_000000000000".to_string(),
                vec![],
            ...
            body: BlockBody {
                transactions: vec![Transaction {
                    hash: "cb_perf_genesis".to_string(),
                    inputs: vec![],
                    outputs: vec![TxOutput {
                        address: "shadow1perf".into(),
                        amount: 10_000,
                        commitment: None,
                        range_proof: None,
                        ephemeral_pubkey: None,
    ...
    fn tx_validation_throughput_10k() {
        ...
        println!(
            "[PERF] 10k TX validation: {} valid in {:.2}ms ({:.0} tx/s)",
            valid_count,
            elapsed.as_millis(),
    ...
    fn mempool_insertion_throughput_5k() {
        ...
        let elapsed = start.elapsed();
        println!(
            "[PERF] Mempool 5k inserts: {} accepted in {:.2}ms",
            accepted,
            elapsed.as_millis()
        );
    ...
    fn utxo_write_throughput_5k() {
        ...
        for i in 0..5_000usize {
            store.add_utxo_str(
                &format!("utxo_perf_{:020}:0", i),
                "owner".into(),
                DUST_LIMIT,
                format!("shadow1perf_{}", i),
            );
        }
        let elapsed = start.elapsed();
    ...
    fn utxo_read_throughput_1k_reads() {
        ...
        for i in 0..1_000usize {
            if store
                .get_utxo_str(&format!("utxo_read_{:020}:0", i))
                .is_some()
            {
                found += 1;
            }
        ...
        let elapsed = start.elapsed();
        println!(
            "[PERF] utxo 1k reads: {} found in {:.2}ms",
            found,
            elapsed.as_millis()
        );
    ...
    fn dag_insert_1000_blocks_timing() {
        ...
        for i in 0..1_000usize {
            ...
            let block = Block {
                header: BlockHeader::new_with_defaults(
                    ...
                    vec![prev.clone()],
                    format!("m_{}", i),
                    ConsensusParams::GENESIS_TIMESTAMP + i as u64,
                    0,
                    ConsensusParams::GENESIS_DIFFICULTY,
                    (i + 1) as u64,
                ),
        ...
        let elapsed = start.elapsed();
        println!(
            "[PERF] DAG 1k blocks: {:.2}ms ({:.0} blocks/s)",
            elapsed.as_millis(),
            1_000.0 / elapsed.as_secs_f64()
        );
        assert!(
            elapsed.as_secs() < 300,
            "DAG 1k blocks took {}s",
            elapsed.as_secs()
        );
    ...
    fn balance_computation_many_utxos() {
        ...
        let elapsed = start.elapsed();
        println!(
            "[PERF] Balance of 500 utxos: {} in {:.2}ms",
            balance,
            elapsed.as_millis()
        );
