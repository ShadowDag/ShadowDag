...
mod tests {
    ...
    fn genesis_block() -> Block {
        Block {
            header: BlockHeader::new_with_defaults(
                1, "rec_genesis_0000000000000".to_string(), vec![],
            ...
            body: BlockBody { transactions: vec![Transaction {
                hash: "cb_rec_genesis".to_string(), inputs: vec![],
                outputs: vec![TxOutput { address: "shadow1rec".into(), amount: 10_000, commitment: None, range_proof: None, ephemeral_pubkey: None }],
    ...
    fn make_block(hash: &str, parent: &str, height: u64) -> Block {
        ...
        Block {
            header: BlockHeader::new_with_defaults(
                1, hash.to_string(), vec![parent.to_string()],
                format!("m_{}", hash), ts, 0,
                ConsensusParams::GENESIS_DIFFICULTY, height,
            ),
            body: BlockBody { transactions: vec![Transaction {
                hash: format!("cb_{}", hash), inputs: vec![],
                outputs: vec![TxOutput { address: "shadow1rec".into(), amount: 1_000, commitment: None, range_proof: None, ephemeral_pubkey: None }],
    ...
    fn make_coinbase_tx(i: usize) -> Transaction {
        let mut tx = Transaction {
            ...
            outputs: vec![TxOutput { address: "shadow1rec_addr".into(), amount: 1_000, commitment: None, range_proof: None, ephemeral_pubkey: None }],
    ...
    fn dag_tips_restored_after_reopen() {
        ...
        }
        {
            ...
            assert!(tips.contains(&"rec_tip_ccccccccccc".to_string()), "Tip must be restored: {:?}", tips);
        }
    }
    ...
    fn utxo_state_persists_after_reopen() {
        ...
        let _ = std::fs::remove_dir_all(path);
        {
            ...
            store.add_utxo_str("rec_utxo_tx:0", "owner_r".into(), 9_876, "shadow1rec_u".into());
        }
        {
            let store = UtxoSet::new(Arc::new(UtxoStore::new(path).expect("open failed")));
    ...
    fn mempool_persists_after_reopen() {
        ...
        { let pool = Mempool::try_new(path).expect("mp"); pool.add_transaction_test(&tx); }
        { let pool = Mempool::try_new(path).expect("mp"); let found = pool.get_transaction(&tx_hash); assert!(found.is_some()); }
    }
    ...
    fn peer_manager_persists_after_reopen() {
        ...
        let _ = std::fs::remove_dir_all(path);
        { let pm = PeerManager::new_default_path(path).unwrap(); pm.add_peer("10.0.0.1:8333").unwrap(); pm.add_peer("10.0.0.2:8333").unwrap(); }
        { let pm = PeerManager::new_default_path(path).unwrap(); let peers = pm.get_peers(); assert!(peers.contains(&"10.0.0.1:8333".to_string())); assert!(peers.contains(&"10.0.0.2:8333".to_string())); }
    }
    ...
    fn ban_persists_after_reopen() {
        ...
        let _ = std::fs::remove_dir_all(path);
        { let pm = PeerManager::new_default_path(path).unwrap(); pm.ban_peer("192.168.5.5:9000", 7200, "malicious"); }
        { let pm = PeerManager::new_default_path(path).unwrap(); assert!(pm.is_banned("192.168.5.5:9000")); }
    }
    ...
    fn multiple_items_no_data_loss_after_reopen() {
        ...
        {
            let store = UtxoSet::new(Arc::new(UtxoStore::new(path).expect("open failed")));
            for i in 0..n { store.add_utxo_str(&format!("{:064x}:0", i), "owner".into(), (i as u64 + 1) * 100, "shadow1ndl".into()); }
        }
        {
            let store = UtxoSet::new(Arc::new(UtxoStore::new(path).expect("open failed")));
    ...
    fn dag_parent_relations_persisted() {
        ...
        let _ = std::fs::remove_dir_all(path);
        { let dag = DagManager::new_required(path).unwrap(); let g = genesis_block(); dag.add_block_validated(&g, true).unwrap(); let b = make_block("rec_par_block_ddddddd", &g.header.hash, 1); dag.add_block_validated(&b, true).unwrap(); }
        { let dag = DagManager::new_required(path).unwrap(); let parents = dag.get_parents("rec_par_block_ddddddd"); assert!(parents.contains(&"rec_genesis_0000000000000".to_string())); }
    }
    ...
    fn dag_child_relations_persisted() {
        ...
        let _ = std::fs::remove_dir_all(path);
        { let dag = DagManager::new_required(path).unwrap(); let g = genesis_block(); dag.add_block_validated(&g, true).unwrap(); let b = make_block("rec_child_block_eeeee", &g.header.hash, 1); dag.add_block_validated(&b, true).unwrap(); }
        { let dag = DagManager::new_required(path).unwrap(); let children = dag.get_children("rec_genesis_0000000000000"); assert!(children.contains(&"rec_child_block_eeeee".to_string())); }
    }
}
