...
mod tests {
    ...
    fn signing_message_differs_for_different_txs() {
        ...
        assert_ne!(
            msg1, msg2,
            "Different TXs must produce different signing messages"
        );
    }
    ...
    fn forged_signature_rejected() {
        ...
        let tx = make_tx("forged_tx", vec![input], vec![make_output("addr1", 50)]);
    ...
    fn invalid_hex_signature_rejected() {
        ...
        let tx = make_tx("bad_hex_tx", vec![input], vec![make_output("addr1", 50)]);
        let mut utxo_set = UtxoSet::new_empty();
        utxo_set.add_test_utxo("utxo1:0", 100, "owner");
        assert!(
            !TxValidator::validate_tx(&tx, &utxo_set),
