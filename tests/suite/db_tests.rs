...
mod tests {
    ...
    fn genesis_block() -> Block {
        Block {
            header: BlockHeader::new_with_defaults(
                1,
                "db_genesis_0000000000000".to_string(),
                vec![],
            ...
            body: BlockBody {
                transactions: vec![Transaction {
                    hash: "cb_db_genesis".to_string(),
                    inputs: vec![],
                    outputs: vec![TxOutput {
                        address: "shadow1db".into(),
                        amount: 10_000,
                        commitment: None,
                        range_proof: None,
    ...
    fn export_all_snapshot_is_complete() {
        ...
        for i in 0..(n / 2) {
            store
                .spend_utxo_str(&format!("snap_tx_{:010}:0", i))
                .unwrap();
        }
    ...
    fn balance_consistent_after_many_operations() {
        ...
        for i in (0..n).step_by(2) {
            store
                .spend_utxo_str(&format!("many_tx_{:020}:0", i))
                .unwrap();
        }
