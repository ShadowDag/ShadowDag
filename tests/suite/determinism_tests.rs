...
mod tests {
    ...
    fn genesis_block() -> Block {
        Block {
            header: BlockHeader::new_with_defaults(
                1, "det_genesis_00000000000000".to_string(), vec![],
            ...
            body: BlockBody { transactions: vec![Transaction {
                hash: "cb_det_genesis".to_string(), inputs: vec![],
                outputs: vec![TxOutput { address: "shadow1det".into(), amount: 10_000, commitment: None, range_proof: None, ephemeral_pubkey: None }],
        ...
        for i in 0..10usize {
            ...
            let block = Block {
                header: BlockHeader::new_with_defaults(
                    ...
                    format!("m_{}_{}", prefix, i), ts,
                    i as u64, ConsensusParams::GENESIS_DIFFICULTY, (i + 1) as u64,
                ),
                body: BlockBody { transactions: vec![Transaction {
                    hash: format!("cb_{}_{}", prefix, i), inputs: vec![],
                    outputs: vec![TxOutput { address: format!("shadow1det_{}", i), amount: 1_000, commitment: None, range_proof: None, ephemeral_pubkey: None }],
    ...
    fn compute_dag_state_root(dag: &DagManager) -> String {
        ...
        for hash in &all { h.update(hash.as_bytes()); }
    ...
    fn same_blocks_produce_same_dag_state_root() {
        ...
        for block in &chain { dag_a.add_block_validated(block, true).unwrap(); }
        let dag_b = tmp_dag("state_root_b");
        for block in &chain { dag_b.add_block_validated(block, true).unwrap(); }
        assert_eq!(compute_dag_state_root(&dag_a), compute_dag_state_root(&dag_b));
    }
    ...
    fn different_blocks_produce_different_state_roots() {
        ...
        for block in &chain_a { dag_a.add_block_validated(block, true).unwrap(); }
        for block in &chain_b { dag_b.add_block_validated(block, true).unwrap(); }
        assert_ne!(compute_dag_state_root(&dag_a), compute_dag_state_root(&dag_b));
    }
    ...
    fn tx_hash_fully_deterministic_across_calls() {
        let tx = Transaction {
            ...
            outputs: vec![TxOutput { address: "shadow1det_hash".into(), amount: 5_000, commitment: None, range_proof: None, ephemeral_pubkey: None }],
    ...
    fn block_hash_matches_reconstructed_hash() {
        let tx = Transaction {
            hash: "preset_hash_for_det_test".to_string(), inputs: vec![],
    ...
    fn signing_message_deterministic() {
        let tx = Transaction {
            hash: "det_sign_tx".to_string(), inputs: vec![],
            outputs: vec![TxOutput { address: "shadow1".into(), amount: 1_000, commitment: None, range_proof: None, ephemeral_pubkey: None }],
    ...
    fn re_applying_same_chain_idempotent_dag_size() {
        ...
        for b in &chain { dag1.add_block_validated(b, true).unwrap(); }
        for b in &chain { dag2.add_block_validated(b, true).unwrap(); }
