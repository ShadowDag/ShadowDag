...
15. **Seven of nine failure-recovery blocks in `FullNode::recompute_virtual_chain` use a best-effort pattern that can leave state neither old nor new.** Only two blocks use the strict pattern (a `restore_failed` flag → `FATAL_reorg_restore_failed_halting` → `std::process::exit(1)`). The other seven discard results — `let _ = utxo_set.rollback_block_undo(...)`, `let _ = utxo_set.apply_block_dag_ordered(...)` for the **old-chain restore** — log a contract-restore failure without acting on it, and then simply `return Err`, with no halt. One of them also lacks an `else` arm when the old block is missing/pruned, so restoration is silently skipped. A failed restore therefore continues execution with an indeterminate UTXO/contract state.
    ...
    *Status: FIXED — all nine blocks now route through one `unwind_reorg_or_halt` helper; a failed restore flushes RocksDB and halts the process rather than returning `Err`. Block reads use a strict accessor so an I/O error is no longer collapsed into "block absent". **Residuals we did not close, and want you to weigh:** (a) the forward old-chain rollback still propagates with bare `?` rather than routing through the helper; (b) a "successful" unwind is not necessarily faithful, because `apply_block_dag_ordered` silently skips duplicate/conflicting transactions; (c) the halt path skips the caller's M3 leaf-cleanup, by design, since we prefer a halt over further mutation; (d) there is still **no automated test that a failed restore actually halts** — the halt is verified by reading, not by execution.*

**Dependency supply chain (self-disclosed)**
16. **`cargo audit` is clean of vulnerabilities but not of warnings.** One real advisory was found and patched for this tag: `crossbeam-epoch` 0.9.18 → 0.9.20 (RUSTSEC-2026-0204, invalid pointer dereference in the `fmt::Pointer` impl), reaching us transitively through `rayon`, not through any consensus path. What remains are `unmaintained`/`unsound` warnings we deliberately did **not** clear:
    ...
    - `rand` 0.9.2 (RUSTSEC-2026-0097, unsound with a custom logger that itself calls `rand::rng()`). The precondition appears not to be met: `telemetry/logging/{logger,structured,log_config}.rs` contain no `rand` reference, and the tree installs no `log`-facade logger at all — there is no `set_logger`, no `set_boxed_logger`, and no `impl log::Log` anywhere outside vendored dependencies. **Confirm this independently rather than take our word**, since this crate feeds key, nonce and decoy selection.
    - Eight gtk-rs GTK3 crates and `fxhash`, all unmaintained. These enter through the wallet-UI tree, not the node. **Question:** should the node binary be feature-gated so a validator build cannot pull a GUI dependency tree at all?
