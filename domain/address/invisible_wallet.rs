...
impl InvisibleWallet {
    ...
    pub fn next_address(&mut self) -> String {
        let child = KeyDerivation::derive_child(&self.spend_scalar.to_bytes(), self.address_index);
...
mod tests {
    ...
    fn ecdh_is_mine_detects_own_output() {
        ...
        let result = StealthAddress::generate_full(&w.view_public, &w.spend_public).unwrap();
    ...
    fn ecdh_is_mine_rejects_other_wallet() {
        ...
        let result = StealthAddress::generate_full(&w1.view_public, &w1.spend_public).unwrap();
