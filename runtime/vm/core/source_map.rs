...
mod tests {
    ...
    fn decode_revert_utf8() {
        assert_eq!(
            SourceMap::decode_revert_reason(b"insufficient balance"),
    ...
    fn decode_revert_hex() {
        assert_eq!(
            SourceMap::decode_revert_reason(&[0xFF, 0x00]),
            "REVERT: 0xff00"
        );
    }
    ...
    fn decode_revert_empty() {
        assert_eq!(SourceMap::decode_revert_reason(&[]), "REVERT (no reason)");
    }
}
