...
pub enum JournalEntry {
    ...
    /// Contract created
    ContractCreated { address: String, code: Vec<u8> },
    ...
    /// Nonce incremented
    NonceIncrement { address: String, old_nonce: u64 },
}
...
impl StateJournal {
    ...
    pub fn logs(&self) -> Vec<&JournalEntry> {
        self.entries
            .iter()
            .filter(|e| matches!(e, JournalEntry::LogEmitted { .. }))
            .collect()
    }
...
mod tests {
    ...
    fn revert_removes_target_checkpoint() {
        ...
        assert_eq!(
            j.checkpoint_count(),
            0,
            "reverted checkpoint must be removed after rollback"
        );
    }
