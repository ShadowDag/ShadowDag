...
pub fn memory_expansion_cost(current_words: u64, new_words: u64) -> u64 {
    ...
    let new_cost =
        new_words * MEMORY_GAS_PER_WORD + (new_words * new_words) / MEMORY_QUADRATIC_DIVISOR;
    let old_cost = current_words * MEMORY_GAS_PER_WORD
        + (current_words * current_words) / MEMORY_QUADRATIC_DIVISOR;
