...
use crate::engine::dag::core::bps_engine::{BpsProfile, BpsParams};
    ...
    pub fn summary(&self) -> String {
        format!(
            "═══ ShadowDAG Network Simulation Results ═══\n\
             ...
             Miners          : {}",
            self.total_blocks,
            self.blue_blocks, self.blue_ratio * 100.0,
            self.red_blocks, (1.0 - self.blue_ratio) * 100.0,
            self.total_txs,
            self.observed_bps,
            self.observed_tps,
            self.avg_dag_width,
...
impl NetworkSimulator {
    ...
    pub fn run(&mut self) -> SimResults {
        ...
        for block_num in 0..total_blocks_target {
            ...
            let parents: Vec<String> = self.tips.iter()
        ...
        let blocks_per_miner: HashMap<u32, u64> = self.miners.iter()
            .map(|m| (m.id, m.blocks))
            .collect();
        ...
        SimResults {
            ...
            avg_dag_width: if width_samples > 0 { total_width as f64 / width_samples as f64 } else { 1.0 },
...
mod tests {
    ...
    fn simulate_32bps() {
        ...
        assert!(results.observed_tps > 5_000.0, "32 BPS with 200 avg_txs should exceed 5K TPS");
        eprintln!("\n{}", results.summary());
    }
    ...
    fn higher_bps_more_throughput() {
        ...
        assert!(r10.total_blocks > r1.total_blocks * 5,
