...
impl TxValidationError {
    ...
    /// Backward compat
    pub fn stage(&self) -> ValidationLayer { self.layer() }
}
...
impl TxValidationPipeline {
    ...
    pub fn validate_syntax(tx: &TxValidationInput) -> TxValidationResult {
        ...
        if tx.output_count == 0 && !tx.is_coinbase  { return Err(TxValidationError::EmptyOutputs); }
        if tx.size_bytes > MAX_TX_SIZE_BYTES        { return Err(TxValidationError::OversizedTransaction); }
        if tx.input_count > MAX_INPUTS              { return Err(TxValidationError::TooManyInputs); }
        if tx.output_count > MAX_OUTPUTS            { return Err(TxValidationError::TooManyOutputs); }

        for &amount in &tx.output_amounts {
            if amount == 0 { return Err(TxValidationError::ZeroOutputAmount); }
