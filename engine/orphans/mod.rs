...
impl OrphanManager {
    ...
    pub fn add(&mut self, hash: &str, parents: Vec<String>, raw_data: &str) -> bool {
        ...
        if raw_data.len() > MAX_ORPHAN_DATA_BYTES {
            return false;
        }
        ...
        self.pending_parents
            .insert(hash.to_string(), parents.iter().cloned().collect());

        self.orphans
            .insert(hash.to_string(), OrphanBlock::new(hash, parents, raw_data));
        true
    }

    pub fn get_ready(&mut self, parent_hash: &str) -> Vec<OrphanBlock> {
        let candidate_hashes: HashSet<String> =
            self.waiting_for.remove(parent_hash).unwrap_or_default();
        ...
        if !ready.is_empty() {
            log::debug!(
                "[Orphans] {} orphan blocks ready for processing",
                ready.len()
            );
        }
        ready
    ...
    pub fn waiting_for_count(&self) -> usize {
