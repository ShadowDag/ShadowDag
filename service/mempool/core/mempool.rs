...
use crate::service::mempool::core::rbf::{MempoolTxInfo, RbfEngine, RbfResult};
...
impl Mempool {
    ...
    fn tx_senders(tx: &Transaction) -> Vec<&str> {
        ...
        let mut senders: Vec<&str> = tx.inputs.iter().map(|inp| inp.owner.as_str()).collect();
...
impl Mempool {
    ...
    pub fn new_with_network<S: Into<SharedDbSource>>(
        source: S,
        network: NetworkMode,
    ...
    pub fn add_transaction_test(&self, tx: &Transaction) -> bool {
        ...
        for input in &tx.inputs {
            ...
            if matches!(self.db.get(parent_key.as_bytes()), Ok(Some(_))) {
                ...
                batch.put(
                    format!("rdep:{}:{}:{}", input.txid, input.index, tx.hash).as_bytes(),
                    b"1",
                );
            }
        }
    ...
    pub fn add_transaction(&self, tx: &Transaction) -> bool {
        ...
        if !conflicting_txs.is_empty() {
            let conflicting_infos: Vec<MempoolTxInfo> = conflicting_txs
                .iter()
            ...
            let confirmed_keys: std::collections::HashSet<String> = tx
                .inputs
                .iter()
                .filter_map(|inp| {
    ...
    pub fn remove_transaction(&self, txid: &str) {
        ...
        if let Some(tx) = self.get_transaction(txid) {
            ...
            let rdep_fwd_keys: Vec<Vec<u8>> = self
                .db
                .prefix_iterator(rdep_prefix.as_bytes())
                .filter_map(|r| r.ok())
    ...
    pub fn evict_to_fit(&self, target_count: usize) {
        ...
        loop {
            ...
            let evict_list: Vec<String> =
                scored.into_iter().map(|(h, _)| h).take(to_evict).collect();
    ...
    pub fn select_transactions_for_block(
        ...
        let rate_map: HashMap<String, f64> = tx_map
            .iter()
    ...
    pub fn flush(&self) {
        ...
        let mempool_keys: Vec<Vec<u8>> = self
            .db
    ...
    pub fn stats(&self) -> MempoolStats {
        ...
        MempoolStats {
            ...
            avg_fee_rate: if total_bytes > 0 {
                total_fees as f64 / total_bytes as f64
            } else {
                0.0
            },
            oldest_ts,
    ...
    pub fn add_transaction_validated(
        ...
        if tx.tx_type == TxType::SwapTx {
            if tx.fee < MIN_RELAY_FEE * 2 {
                return Err(MempoolError::FeeTooLow {
                    fee: tx.fee,
                    minimum: MIN_RELAY_FEE * 2,
                });
            }
            if tx.payload_hash.is_none() {
                return Err(MempoolError::ValidationFailed(
                    "SwapTx missing HTLC secret hash".into(),
                ));
            }
        }
        if tx.tx_type == TxType::DexOrder {
            ...
            if tx.payload_hash.is_none() {
                return Err(MempoolError::ValidationFailed(
                    "DexOrder missing order payload".into(),
                ));
            }
        }
        ...
        for sender in Self::tx_senders(tx) {
            if self.sender_tx_count(sender) >= MAX_TXS_PER_SENDER {
                return Err(MempoolError::Other(format!(
                    "sender {} already has {} unconfirmed TXs in mempool",
...
mod tests {
    ...
    fn conflict_detection_blocks_double_spend() {
        ...
        assert!(
            ...
            "Second TX spending same input must be rejected"
        );
    }
    ...
    fn evict_expired_removes_old_txs() {
        ...
        mp.evict_expired();
        assert!(
            mp.get_transaction(old_hash).is_none(),
            "Expired TX must be removed"
        );
    }
...
mod cpfp_tests {
    ...
    fn test_descendant_fee_includes_children() {
        ...
        assert_eq!(
            child_desc, 200,
            "Leaf TX descendant fee should be its own fee"
        );
    }
...
mod policy_tests {
    ...
    fn surge_pricing_empty_pool() {
        ...
        assert!(
            (rate - MIN_FEE_RATE).abs() < 1e-9,
            "Empty pool should use base rate"
        );
    }
            ...
            .filter_map(|(k, _)| {
                ...
                let receive_time = match self.db.get(ts_key.as_bytes()) {
                    ...
                    _ => {
                        ...
                        let tx: Transaction = bincode::deserialize(&self.db.get(&k).ok()??).ok()?;
                        tx.timestamp
                    }
                };
