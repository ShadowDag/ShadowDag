    ...
    pub fn insert(&mut self, entry: MempoolEntry) -> Result<(), MempoolError> {
        for key in &entry.input_keys {
            if let Some(existing) = self.spend_index.get(key) {
                return Err(MempoolError::ConflictingInput(format!(
                    "double spend: input {} already used by tx {}",
                    key, existing
                )));
            }
        }
...
mod tests {
    ...
    fn top_by_fee_returns_highest_first() {
        let mut idx = MempoolIndex::new();
        idx.insert(MempoolEntry::new("low", 10, 200, vec!["u1:0".into()], 1000))
            .unwrap();
        idx.insert(MempoolEntry::new(
            "high",
            500,
            200,
            vec!["u2:0".into()],
    ...
    fn total_fees_sums_correctly() {
        let mut idx = MempoolIndex::new();
        idx.insert(MempoolEntry::new("a", 100, 200, vec!["ua:0".into()], 1000))
            .unwrap();
        idx.insert(MempoolEntry::new("b", 200, 200, vec!["ub:0".into()], 1000))
            .unwrap();
    ...
    fn evict_older_than_removes_old() {
        let mut idx = MempoolIndex::new();
        idx.insert(MempoolEntry::new("old", 100, 200, vec!["uo:0".into()], 500))
            .unwrap();
        idx.insert(MempoolEntry::new(
            "new",
            100,
