...
pub enum TxDirection { Sent, Received }

#[derive(Debug, Clone, Serialize, Deserialize, PartialEq)]
pub enum TxStatus { Pending, Confirmed, Failed }
...
impl Wallet {
    ...
    pub fn unlock(&mut self, enc_seed: &EncryptedSeed, password: &str) -> Result<(), WalletError> {
        let seed = decrypt_bytes(&enc_seed.ciphertext, &enc_seed.salt, &enc_seed.nonce, password)?;
        self.session_key = Some(seed);
        self.locked = false;
        Ok(())
    }
    ...
    pub fn add_account(&mut self, index: u32, label: &str) -> Result<WalletAccount, WalletError> {
        ...
        let seed = self.session_key.as_ref().ok_or(WalletError::Other("No session key".to_string()))?;
    ...
    pub fn derive_change_address(&mut self, account: u32, addr_index: u32) -> Result<WalletAddress, WalletError> {
        if self.locked { return Err(WalletError::Locked); }
        let seed = self.session_key.as_ref().ok_or(WalletError::Other("No session key".to_string()))?;
    ...
    pub fn accounts(&self) -> &[WalletAccount] { &self.state.accounts }
    ...
    pub fn history_for(&self, address: &str) -> Vec<&WalletTx> {
        let mut txs: Vec<&WalletTx> = self.history.get(address)
    ...
    pub fn update_utxos(&mut self, address: &str, utxos: Vec<Walletutxo>) {
        let bal: u64 = utxos.iter().filter(|u| !u.is_locked).map(|u| u.amount)
            .try_fold(0u64, |acc, u| acc.checked_add(u))
            .unwrap_or(u64::MAX);
        self.utxos.insert(address.to_string(), utxos);

        if let Some(acc) = self.state.accounts.iter_mut()
    ...
    pub fn build_tx(
        ...
        for u in all_utxos {
            ...
            total_in += u.amount;
            if total_in >= amount.saturating_add(fee) { break; }
        }

        let change = total_in.saturating_sub(amount.saturating_add(fee));
        let seed   = self.session_key.as_ref().ok_or(WalletError::Other("No session key".to_string()))?.clone();
...
fn derive_key(seed: &[u8], account: u32, index: u32, change: bool) -> Result<SigningKey, WalletError> {
    use hmac::{Hmac, Mac};
    type HmacSha256 = Hmac<Sha256>;

    let path = format!("ShadowDAG/44'/999'/{}'/{}/{}", account, if change { 1 } else { 0 }, index);
    let mut mac = <HmacSha256 as Mac>::new_from_slice(seed).map_err(|e| WalletError::KeyDerivation(e.to_string()))?;
    mac.update(path.as_bytes());
    let res = mac.finalize().into_bytes();
    let key: [u8; 32] = res[..32].try_into().map_err(|_| WalletError::KeyDerivation("Key slice error".to_string()))?;
...
fn encrypt_bytes(data: &[u8], password: &str) -> Result<EncryptedSeed, WalletError> {
    ...
    let ct     = cipher.encrypt(n, data).map_err(|_| WalletError::Encryption("Encrypt failed".to_string()))?;
    key.zeroize();

    Ok(EncryptedSeed { ciphertext: ct, salt, nonce })
}

fn decrypt_bytes(ct: &[u8], salt: &[u8], nonce: &[u8], password: &str) -> Result<Vec<u8>, WalletError> {
    let mut key = [0u8; 32];
    pbkdf2_hmac::<Sha256>(password.as_bytes(), salt, PBKDF2_ITER, &mut key);
    let cipher = Aes256Gcm::new_from_slice(&key).map_err(|e| WalletError::Encryption(e.to_string()))?;
    let n      = aes_gcm::Nonce::from_slice(nonce);
