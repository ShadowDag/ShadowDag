...
impl RpcState {
    ...
    pub fn new(db: Arc<DB>) -> Result<Self, NetworkError> {
        ...
        let utxo_store =
            UtxoSet::new(store as Arc<dyn crate::domain::traits::utxo_backend::UtxoBackend>);
        let mempool = Mempool::new(db.clone()).map_err(|e| NetworkError::Other(e.to_string()))?;
        let peer_manager = Arc::new(PeerManager::new_default().map_err(|e| {
            NetworkError::Other(format!(
                "DEPRECATED peer DB init (mainnet default) — use RpcServer::new_for_network instead. Error: {}",
                e
            ))
        })?);
        Ok(Self {
    ...
    pub fn new_with_peers_path(
        ...
        let peer_manager = match PeerManager::new(peers_path) {
            Some(pm) => Arc::new(pm),
            None => Arc::new(
                PeerManager::new_default()
                    .map_err(|e| NetworkError::Other(format!("PeerManager init failed: {}", e)))?,
            ),
        };
        Self::new_with_peer_manager(peer_manager, db, data_dir)
    }
...
impl RpcServer {
    ...
    pub fn new_for_network(
        ...
        Ok(Self {
            state: Arc::new(Mutex::new(RpcState::new_with_peers_path(
                peers_path, db, data_dir,
            )?)),
            port,
            rate_table: Arc::new(Mutex::new(HashMap::new())),
            running: Arc::new(AtomicBool::new(false)),
    ...
    pub fn new_for_network_with_peer_manager(
        port: u16,
        peer_manager: Arc<PeerManager>,
        db: Arc<DB>,
        data_dir: Option<&std::path::Path>,
        ...
        Ok(Self {
            state: Arc::new(Mutex::new(RpcState::new_with_peer_manager(
                peer_manager,
                db,
                data_dir,
            )?)),
    ...
    pub fn new_with_max_request_size(
        port: u16,
        max_size: usize,
        db: Arc<DB>,
    ) -> Result<Self, NetworkError> {
    ...
    pub fn start(&self) -> Result<(), NetworkError> {
        ...
        let listener = TcpListener::bind(&addr).map_err(|e| {
            NetworkError::ConnectionFailed(format!("RPC bind {} failed: {}", addr, e))
        })?;
    ...
    fn handle_connection(
        ...
        let resp_json =
            serde_json::to_value(&response).unwrap_or_else(|_| json!({"error": "internal"}));
    ...
    fn write_http_response(
        ...
        let body_str =
            serde_json::to_string(&body).map_err(|e| NetworkError::Serialization(e.to_string()))?;
        let resp = format!(
    ...
    fn dispatch(method: &str, params: Vec<Value>, id: Value, state: &SharedState) -> RpcResponse {
        match method {
            "login" => {
                let params_obj = params.first().cloned().unwrap_or(json!({}));
                let username = params_obj
                    .get("username")
                    .and_then(|v| v.as_str())
                    .unwrap_or("");
                let password = params_obj
                    .get("password")
                    .and_then(|v| v.as_str())
                    .unwrap_or("");
                match state.lock() {
                    Ok(mut s) => match s.auth_manager.login(username, password) {
    ...
    fn cmd_getblocks(params: Vec<Value>, id: Value, state: &SharedState) -> RpcResponse {
        let start_height = params.first().and_then(|v| v.as_u64()).unwrap_or(0);
        let count_raw = params.get(1).and_then(|v| v.as_u64()).unwrap_or(10) as usize;
    ...
    fn cmd_sendrawtransaction(params: Vec<Value>, id: Value, state: &SharedState) -> RpcResponse {
        ...
        let tx: Transaction = match serde_json::from_str(raw) {
            Ok(t) => t,
            Err(e) => {
                return RpcResponse::err(id, ERR_INVALID_TX, format!("Invalid TX JSON: {}", e))
            }
        };
    ...
    fn cmd_gettxinfo(params: Vec<Value>, id: Value, state: &SharedState) -> RpcResponse {
        ...
        match state.lock() {
            Ok(s) => match s.mempool.get_transaction(&hash) {
                ...
                None => RpcResponse::err(id, ERR_NOT_FOUND, format!("TX {} not found", hash)),
            },
            Err(_) => RpcResponse::err(id, ERR_INTERNAL, "State lock error"),
        }
    }
    ...
    fn cmd_submitblock(params: Vec<Value>, id: Value, _state: &SharedState) -> RpcResponse {
        ...
        let transactions: Vec<Transaction> =
            match block_json.get("transactions").and_then(|v| v.as_array()) {
                Some(arr) => {
        ...
        if let Err(rej) = DagShield::pre_validate_block(&block) {
            return RpcResponse::err(
                ...
                ERR_INVALID_PARAMS,
                format!(
                    "Block rejected by DagShield: {} (ban_score={})",
                    rej.reason, rej.ban_score
                ),
            );
        }
    ...
    fn cmd_validateaddress(params: Vec<Value>, id: Value, state: &SharedState) -> RpcResponse {
        ...
        let valid = (address.starts_with("SD1")
    ...
    fn cmd_getblocksbyheight(params: Vec<Value>, id: Value, state: &SharedState) -> RpcResponse {
        ...
        match state.lock() {
            Ok(s) => {
                ...
                let blocks: Vec<Value> = hashes
                    .iter()
                    .filter_map(|h| {
                        s.block_store.get_block(h).map(|b| {
                            json!({
                                "hash":      b.header.hash,
    ...
    fn cmd_getsnapshotinfo(id: Value, state: &SharedState) -> RpcResponse {
        use crate::engine::state_snapshot::{MAX_SNAPSHOTS, SNAPSHOT_INTERVAL_BLOCKS};
        match state.lock() {
            Ok(s) => {
        ...
        match state.lock() {
            Ok(s) => match s.block_store.get_block(&hash) {
                Some(block) => {
                    ...
                    let children: Vec<String> = candidates
                        .into_iter()
                        .filter(|child_hash| {
                            s.block_store
                                .get_block(child_hash)
    ...
    fn cmd_getdifficulty(id: Value, state: &SharedState) -> RpcResponse {
        match state.lock() {
            Ok(s) => {
                ...
                let target =
                    crate::engine::mining::pow::pow_validator::PowValidator::difficulty_to_target(
                        difficulty,
                    );
                RpcResponse::ok(
                    id,
        ...
        match state.lock() {
            Ok(s) => match s.block_store.get_block(hash) {
                Some(b) => {
                    ...
                    RpcResponse::ok(
                        id,
                        json!({"hash": hash, "size_bytes": size, "tx_count": b.body.transactions.len()}),
                    )
                }
                None => RpcResponse::err(id, ERR_NOT_FOUND, "Block not found"),
            },
        ...
        match state.lock() {
            Ok(s) => match s.block_store.get_block(hash) {
                Some(b) => {
                    ...
                    RpcResponse::ok(
                        id,
                        json!({"hash": hash, "tx_count": txids.len(), "txids": txids}),
                    )
                }
                None => RpcResponse::err(id, ERR_NOT_FOUND, "Block not found"),
            },
            ...
            Ok(s) => match s.block_store.get_block(hash) {
                Some(b) => match bincode::serialize(&b) {
                    Ok(data) => RpcResponse::ok(
                        id,
                        json!({"hash": hash, "hex": hex::encode(&data), "size": data.len()}),
                    ),
    ...
    fn cmd_getblockparents(params: Vec<Value>, id: Value, state: &SharedState) -> RpcResponse {
        ...
        match state.lock() {
            Ok(s) => match s.block_store.get_block(hash) {
                Some(b) => RpcResponse::ok(
                    id,
                    json!({"hash": hash, "parents": b.header.parents, "parent_count": b.header.parents.len()}),
                ),
                None => RpcResponse::err(id, ERR_NOT_FOUND, "Block not found"),
            },
            Err(_) => RpcResponse::err(id, ERR_INTERNAL, "State lock error"),
    ...
    fn cmd_decodetransaction(params: Vec<Value>, id: Value) -> RpcResponse {
        ...
        match hex::decode(hex_data) {
            Ok(data) => match bincode::deserialize::<
                crate::domain::transaction::transaction::Transaction,
            >(&data)
            {
                Ok(tx) => RpcResponse::ok(
                    id,
                    json!({
    ...
    fn cmd_gettxout(params: Vec<Value>, id: Value, state: &SharedState) -> RpcResponse {
        ...
        match state.lock() {
            Ok(s) => match s.utxo_store.get_utxo(&key) {
                Some(u) => RpcResponse::ok(
                    id,
                    json!({"txid": txid, "index": index, "amount": u.amount, "address": u.address, "spent": u.spent}),
                ),
    ...
    fn cmd_getaddednodeinfo(id: Value, state: &SharedState) -> RpcResponse {
        match state.lock() {
            Ok(s) => RpcResponse::ok(
                id,
                json!({"peer_count": s.peer_manager.get_peers().len(), "max_peers": ConsensusParams::MAX_PEERS}),
            ),
            Err(_) => RpcResponse::err(id, ERR_INTERNAL, "State lock error"),
        }
    }
    ...
    fn cmd_getmineraddress(id: Value) -> RpcResponse {
        RpcResponse::ok(
            id,
            json!({"address": ConsensusParams::OWNER_REWARD_ADDRESS, "type": "developer_fund"}),
        )
    }
    ...
    fn cmd_getcoinbasematurity(id: Value) -> RpcResponse {
        RpcResponse::ok(
            id,
            json!({"maturity": ConsensusParams::COINBASE_MATURITY, "blocks": ConsensusParams::COINBASE_MATURITY}),
        )
    }
    ...
    fn cmd_getbluescore(params: Vec<Value>, id: Value, state: &SharedState) -> RpcResponse {
        ...
        match state.lock() {
            Ok(s) => match s.block_store.get_block(hash) {
                Some(b) => {
                    RpcResponse::ok(id, json!({"hash": hash, "blue_score": b.header.blue_score}))
                }
                None => RpcResponse::err(id, ERR_NOT_FOUND, "Block not found"),
            },
            Err(_) => RpcResponse::err(id, ERR_INTERNAL, "State lock error"),
    ...
    fn cmd_getselectedparent(params: Vec<Value>, id: Value, state: &SharedState) -> RpcResponse {
        ...
        match state.lock() {
            Ok(s) => match s.block_store.get_block(hash) {
                Some(b) => RpcResponse::ok(
                    id,
                    json!({"hash": hash, "selected_parent": b.header.selected_parent}),
                ),
                None => RpcResponse::err(id, ERR_NOT_FOUND, "Block not found"),
            },
            Err(_) => RpcResponse::err(id, ERR_INTERNAL, "State lock error"),
        ...
        match state.lock() {
            Ok(s) => match s.block_store.get_block(hash) {
                Some(b) => {
                    ...
                    RpcResponse::ok(
                        id,
                        json!({"hash": hash, "height": b.header.height, "dag_width": width, "anticone_estimate": width.saturating_sub(1)}),
                    )
                }
                None => RpcResponse::err(id, ERR_NOT_FOUND, "Block not found"),
            },
    ...
    fn cmd_help(params: Vec<Value>, id: Value) -> RpcResponse {
        ...
        RpcResponse::ok(
            id,
            json!({"method": method, "hint": "Use getrpcmethods for full list"}),
        )
    }
    ...
    fn cmd_getgasprice(id: Value, state: &SharedState) -> RpcResponse {
        match state.lock() {
            Ok(s) => {
                let pool_usage =
                    s.mempool.count() as f64 / ConsensusParams::MAX_MEMPOOL_SIZE as f64;
                let gas_price = if pool_usage > 0.8 {
                    10
                } else if pool_usage > 0.5 {
                    5
                } else {
                    1
                };
                RpcResponse::ok(
                    id,
                    json!({"gas_price": gas_price, "pool_usage_pct": (pool_usage * 100.0) as u64}),
                )
            }
            Err(_) => RpcResponse::err(id, ERR_INTERNAL, "State lock error"),
        }
    ...
    fn cmd_getcodelength(id: Value) -> RpcResponse {
        RpcResponse::ok(
            id,
            json!({"max_code_size": 24576, "max_init_code_size": 49152}),
        )
    }
    ...
    fn cmd_getdagslice(params: Vec<Value>, id: Value, state: &SharedState) -> RpcResponse {
        ...
        match state.lock() {
            Ok(s) => {
                ...
                RpcResponse::ok(
                    id,
                    json!({"from": from_height, "layers": layers, "count": layers.len()}),
                )
            }
            Err(_) => RpcResponse::err(id, ERR_INTERNAL, "State lock error"),
        }
    ...
    fn cmd_getblockneighbors(params: Vec<Value>, id: Value, state: &SharedState) -> RpcResponse {
        ...
        match state.lock() {
            Ok(s) => match s.block_store.get_block(hash) {
                Some(b) => {
                    let child_candidates = s
                        .block_store
                        .get_block_hashes_at_height(b.header.height + 1);
                    // Filter to actual children (blocks whose parents include this hash)
                    let children: Vec<String> = child_candidates
                        .into_iter()
                        .filter(|ch| {
                            s.block_store
                                .get_block(ch)
    ...
    fn cmd_getcheckpoints(id: Value) -> RpcResponse {
        ...
        let cps: Vec<Value> = all
            .iter()
            .map(|cp| json!({"height": cp.height, "hash": cp.hash}))
            .collect();
    ...
    fn cmd_getfinalityinfo(id: Value, state: &SharedState) -> RpcResponse {
        match state.lock() {
            Ok(s) => {
                use crate::engine::consensus::finality::{
                    BASE_FINALITY_DEPTH, CHECKPOINT_INTERVAL, MAX_FINALITY_DEPTH,
                };
                use crate::engine::consensus::reorg::{
                    ECONOMIC_FINALITY_WORK, FINALITY_DEPTH, MAX_REORG_DEPTH,
                };
                RpcResponse::ok(
                    id,
                    json!({
    ...
    fn cmd_getminingprofiles(id: Value) -> RpcResponse {
        use crate::engine::dag::core::bps_engine::BpsParams;
        let profiles: Vec<Value> = [1u32, 10, 32, 64]
            .iter()
            .map(|&bps| {
                let p = BpsParams::for_bps(bps);
                json!({
    ...
    fn cmd_getscratchpadinfo(id: Value) -> RpcResponse {
        use crate::engine::mining::algorithms::shadowhash::{MIX_ROUNDS, SCRATCHPAD_SIZE};
        RpcResponse::ok(
            id,
            json!({
    ...
    fn cmd_getblockrange(params: Vec<Value>, id: Value, state: &SharedState) -> RpcResponse {
        ...
        match state.lock() {
            Ok(s) => {
                ...
                RpcResponse::ok(
                    id,
                    json!({"from": from, "to": to, "blocks": blocks, "count": blocks.len()}),
                )
            }
            Err(_) => RpcResponse::err(id, ERR_INTERNAL, "State lock error"),
        }
    ...
    fn cmd_getrelayinfo(id: Value) -> RpcResponse {
        use crate::service::network::relay::compact_block::{MAX_PREFILLED, SHORT_ID_BYTES};
        RpcResponse::ok(
            id,
            json!({
            ...
            Ok(s) => match s.block_store.get_block(block_hash) {
                Some(b) => {
                    let tx_hashes: Vec<String> =
                        b.body.transactions.iter().map(|t| t.hash.clone()).collect();
                    match crate::domain::block::merkle_tree::MerkleTree::generate_proof(
                        &tx_hashes, tx_index,
                    ) {
                        Some(proof) => {
                            let proof_hashes: Vec<String> =
                                proof.iter().map(|(h, _)| hex::encode(h)).collect();
    ...
    fn cmd_getfeeestimate(id: Value, state: &SharedState) -> RpcResponse {
        match state.lock() {
            Ok(s) => {
                let estimate =
                    crate::service::mempool::fees::fee_market::FeeMarket::estimate_fee(&s.mempool);
                RpcResponse::ok(
                    id,
                    json!({
    ...
    fn cmd_getfeestats(id: Value, state: &SharedState) -> RpcResponse {
        match state.lock() {
            Ok(s) => {
                let ratio = crate::service::mempool::fees::fee_market::FeeMarket::congestion_ratio(
                    &s.mempool,
                );
                RpcResponse::ok(
                    id,
    ...
    fn cmd_call_contract(params: Vec<Value>, id: Value, state: &SharedState) -> RpcResponse {
        ...
        let result =
            executor.simulate_call(contract_addr, &calldata, caller, value, gas_limit, 0, "");
    ...
    fn cmd_get_transaction_receipt(
        params: Vec<Value>,
        id: Value,
        state: &SharedState,
    ) -> RpcResponse {
    ...
    fn cmd_get_logs(params: Vec<Value>, id: Value, state: &SharedState) -> RpcResponse {
        ...
        let block_hash = if let Some(h) = params.first().and_then(|v| v.as_str()) {
            ...
            return RpcResponse::err(
                id,
                ERR_INVALID_PARAMS,
                "block_hash or block_height required",
            );
        };
    ...
    fn cmd_verify_contract(params: Vec<Value>, id: Value, state: &SharedState) -> RpcResponse {
        ...
        let package = match ContractPackage::from_json(package_json) {
            Ok(p) => p,
            Err(e) => {
                return RpcResponse::err(id, ERR_INVALID_PARAMS, format!("invalid package: {}", e))
            }
        };
    ...
    fn cmd_get_contract_info(params: Vec<Value>, id: Value, state: &SharedState) -> RpcResponse {
        ...
        let code_size = storage
            .get_state(&format!("code:{}", address))
            .and_then(|h| hex::decode(&h).ok())
            .map(|c| c.len())
            .unwrap_or(0);

        let vm_ver = storage
            .get_state(&format!("vm_version:{}", address))
...
mod tests {
    ...
    fn getminerinfo_returns_reward() {
        ...
        assert_eq!(
            r["result"]["block_reward"],
            json!(ConsensusParams::BLOCK_REWARD)
        );
    }
