...
mod tests {
    ...
    fn rate_meter_uses_actual_elapsed_between_samples() {
        ...
        let mut meter = RateMeter::new(60);
        meter.samples.push_back(MetricPoint { timestamp: 1000, value: 5.0 });
        meter.samples.push_back(MetricPoint { timestamp: 1010, value: 5.0 });
        // total = 10, elapsed = 10s → rate = 1.0 events/sec
        let r = meter.rate();
        assert!((r - 1.0).abs() < 1e-9,
            "expected rate ~1.0/s for 10 events over 10 seconds, got {}", r);
    }
    ...
    fn rate_meter_zero_elapsed_falls_back_to_nominal_window() {
        ...
        let mut meter = RateMeter::new(60);
        meter.samples.push_back(MetricPoint { timestamp: 1000, value: 3.0 });
        meter.samples.push_back(MetricPoint { timestamp: 1000, value: 4.0 });
        // total = 7, elapsed = 0 → fall back to 60s → rate = 7/60
        let r = meter.rate();
        assert!((r - (7.0 / 60.0)).abs() < 1e-9,
            "expected fallback to nominal 60s window, got {}", r);
    }
